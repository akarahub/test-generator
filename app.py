...
def create_test_folders(train_folder, test_folder, test_ratio=0.2):
    ...
    subfolders = [d for d in os.listdir(train_folder) if os.path.isdir(
        # Exclude "." and ".."
        os.path.join(train_folder, d)) and d != "." and d != ".."]
    ...
    for folder in subfolders:
        ...
        if not os.path.exists(test_folder_path):
            try:
                ...
                logger.error(
                    f"Error creating directory {test_folder_path}: {e}")
                return

    # Randomly select images to move to test folders
    for folder in subfolders:
        folder_path = os.path.join(train_folder, folder)
        images = [f for f in os.listdir(folder_path) if os.path.isfile(os.path.join(
            # Filter for image files
            folder_path, f)) and f.lower().endswith((".png", ".jpg", ".jpeg"))]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-tr", "--train_folder",
                        help="Path to the main train folder.")
    parser.add_argument("-te", "--test_folder",
                        help="Path to the destination test folder.")
